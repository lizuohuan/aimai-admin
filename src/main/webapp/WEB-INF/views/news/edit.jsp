...
<div style="margin: 15px;">
    ...
    <fieldset class="layui-elem-field layui-field-title" style="margin-top: 20px;">
        <legend>更新新闻资讯</legend>
    </fieldset>
    <form class="layui-form" action="" id="formData" ng-controller="editNewsCtr" ng-cloak>
        ...
        <div class="layui-form-item">
            ...
            <div class="layui-input-block" id="preview">
                <div class="preview" ng-if="news.image != null">
                    <img src="{{imgUrl}}/{{news.image}}"><br>
                    <button type="button" onclick="deleteImg(this)"
                            image="{{news.image}}" class="layui-btn layui-btn-danger"><i class="layui-icon">&#xe640;</i>删除</button>
                </div>
            </div>
        </div>
        ...
        <div class="layui-form-item">
            ...
            <div class="layui-input-block">
                <input type="text" value="{{news.title}}" name="title" lay-verify="required" placeholder="请输入标题" autocomplete="off" class="layui-input" maxlength="50">
            </div>
        </div>
        ...
        <div class="layui-form-item">
            ...
            <div class="layui-input-block">
                <input type="text" name="editor" value="{{news.editor}}" placeholder="请输入责任编辑人" autocomplete="off" class="layui-input" maxlength="50">
            </div>
        </div>
        <div class="layui-form-item">
            <label class="layui-form-label">摘要</label>
            <div class="layui-input-block">
                <input type="text" name="digest" value="{{news.digest}}" placeholder="请输入摘要" autocomplete="off" class="layui-input" maxlength="50">
            </div>
        </div>
        ...
        <div class="layui-form-item">
            ...
            <div class="layui-input-block">
                <input type="text" value="{{news.source}}" name="source" placeholder="请输入来源" autocomplete="off" class="layui-input" maxlength="50">
            </div>
        </div>
        <div class="layui-form-item">
        ...
        <div class="layui-form-item">
            ...
            <div class="layui-input-block">
                <input type="text" value="{{news.linkUrl}}" name="linkUrl" placeholder="请输入外链地址" autocomplete="off" class="layui-input" maxlength="50">
            </div>
        </div>
        ...
        <input type="hidden" name="image" value="{{news.image}}" id="image">
                <input type="hidden" name="cityId" value="{{news.cityId}}">
        <div class="layui-form-item">
            <div class="layui-input-block">
...
<script>
    ...
    webApp.controller("editNewsCtr", function($scope,$http,$timeout){
        $scope.news = null; //banner信息
        $scope.newsId = AM.getUrlParam("id");
        $scope.imgUrl = AM.ipImg;
        AM.ajaxRequestData("post", false, AM.ip + "/news/info", {id : $scope.newsId} , function(result) {
            if (result.flag == 0 && result.code == 200) {
                $scope.news = result.data;
                $(document).ready(function(){
                    var ue = UE.getEditor('editor');
                    ue.ready(function() {//编辑器初始化完成再赋值
                        ue.setContent($scope.news.content);  //赋值给UEditor
                    });
        ...
        layui.use(['form', 'layedit', 'laydate', 'upload', 'layedit'], function() {
            ...
            if ($scope.news.cityJsonAry != null && $scope.news.cityJsonAry != "") {
                selectProvince($scope.news.cityJsonAry[0]); //省
                selectCity($scope.news.cityJsonAry[0], $scope.news.cityJsonAry[1]); //市
                selectCounty($scope.news.cityJsonAry[1], $scope.news.cityJsonAry[2]); //区
            } else {
                selectProvince(0); //默认的省
            }

            if ($scope.news.image != null) {
                $("#uploadImgDiv").hide();
            }
            //选中是否推荐
            $("input[name='isRecommend']").each(function () {
                if (Number($(this).val()) == Number($scope.news.isRecommend)) {
                    $(this).click();
                }
            });
            ...
            $("input[name='isLink']").each(function () {
                if (Number($(this).val()) == Number($scope.news.isLink)) {
                    $(this).click();
                }
            });
            ...
            form.on('submit(demo1)', function(data) {
                ...
                AM.ajaxRequestData("post", false, AM.ip + "/news/update", data.field , function(result) {
                    if (result.flag == 0 && result.code == 200) {
                        layer.alert('修改成功.', {
