    ...
    <form class="layui-form" action="" id="formData" ng-controller="editBannerCtr" ng-cloak>
        ...
        <div class="layui-form-item">
            ...
            <div class="layui-input-block" id="previewApp">
                <div class="preview" ng-if="banner.apiImage != null && banner.apiImage != ''">
                    <img src="{{imgUrl}}/{{banner.apiImage}}"><br>
                    <button type="button" onclick="deleteImg(this,0)"
                            apiImage="{{banner.apiImage}}" class="layui-btn layui-btn-danger"><i class="layui-icon">&#xe640;</i>删除</button>
                </div>
            </div>
        </div>
        ...
        <div class="layui-form-item"">
            ...
            <div class="layui-input-block" id="previewPC">
                <div class="preview" ng-if="banner.pcImage != null && banner.pcImage != ''">
                    <img src="{{imgUrl}}/{{banner.pcImage}}"><br>
                    <button type="button" onclick="deleteImg(this,1)"
                            pcImage="{{banner.pcImage}}" class="layui-btn layui-btn-danger"><i class="layui-icon">&#xe640;</i>删除</button>
                </div>
            </div>
        </div>
        ...
        <div class="layui-form-item">
            ...
            <div class="layui-input-inline">
                <select name="isBanner" lay-verify="required" lay-filter="isBanner" lay-search>
        ...
        <div class="layui-form-item">
            ...
            <div class="layui-input-block">
                <input type="text" value="{{banner.title}}" name="title" lay-verify="required" placeholder="请输入标题" autocomplete="off" class="layui-input" maxlength="50">
            </div>
        </div>
        ...
        <div class="layui-form-item">
            ...
            <div class="layui-input-block">
                <input type="text" name="editor" value="{{banner.editor}}" placeholder="请输入责任编辑人" autocomplete="off" class="layui-input" maxlength="50">
            </div>
        </div>
        ...
        <div class="layui-form-item">
            ...
            <div class="layui-input-block">
                <input type="text" value="{{banner.source}}" name="source" placeholder="请输入来源" autocomplete="off" class="layui-input" maxlength="50">
            </div>
        </div>
        ...
        <div class="layui-form-item">
            ...
            <div class="layui-input-block">
                <input type="text" value="{{banner.linkUrl}}" name="linkUrl" placeholder="请输入外链地址" autocomplete="off" class="layui-input" maxlength="50">
            </div>
        </div>
        <div class="layui-form-item">
            <label class="layui-form-label">到期时间<span class="font-red">*</span></label>
            <div class="layui-input-inline">
                <input type="text" id="validitys" value="{{banner.validity | date:'yyyy-MM-dd HH:mm:ss'}}" name="validitys"  readonly lay-verify="" placeholder="请输入到期时间" autocomplete="off" class="layui-input" maxlength="20">
            </div>
        </div>

        <input type="hidden" name="apiImage" value="{{banner.apiImage}}" id="apiImage">
        <input type="hidden" name="pcImage" value="{{banner.pcImage}}" id="pcImage">
...
<script>
    ...
    webApp.controller("editBannerCtr", function($scope,$http,$timeout){
        ...
        $scope.bannerId = AM.getUrlParam("id");
        $scope.imgUrl = AM.ipImg;
        AM.ajaxRequestData("post", false, AM.ip + "/banner/info", {id : $scope.bannerId} , function(result) {
            if (result.flag == 0 && result.code == 200) {
                $scope.banner = result.data;
//                ueEditor.setContent($scope.banner.content);
            }
        });
        $(document).ready(function(){
            var ue = UE.getEditor('editor');
            ue.ready(function() {//编辑器初始化完成再赋值
                ue.setContent($scope.banner.content);  //赋值给UEditor
            });
        ...
        layui.use(['form', 'layedit', 'laydate', 'upload', 'layedit'], function() {
            ...
            //选中类型
            $("select[name='isBanner'] option").each(function () {
                if ($(this).val() == $scope.banner.isBanner) {
            ...
            form.on('select(isBanner)', function(data) {
                if (data.value == 0) {
                    ...
                    $("#spanImgApp").html("*");
                    $("#locationDiv").removeAttr("lay-verify");
                }
                form.render();
            });
            ...
            if ($scope.banner.isBanner == 0) {
                //选中类型
                $("select[name='location'] option").each(function () {
                    if ($(this).val() == $scope.banner.location) {
                        $(this).attr("selected", true);
                    }
                    form.render();
                });
                $("#locationDiv").show();
                $("#isBanner").attr("lay-verify","required");
            }

            if ($scope.banner.apiImage != null && $scope.banner.apiImage != "" ) {
                $("#uploadImgDivApp").hide();
            } else {
                $("#previewApp").parent().hide();
            }
            if ($scope.banner.pcImage != null  && $scope.banner.pcImage != "") {
                $("#uploadImgDivPC").hide();
            } else {
                $("#previewPC").parent().hide();}

            //是否是外链
            $("input[name='isLink']").each(function () {
                if (Number($(this).val()) == Number($scope.banner.isLink)) {
                    $(this).click();
                }
            });
            ...
            form.on('submit(demo1)', function(data) {
                ...
                AM.ajaxRequestData("post", false, AM.ip + "/banner/update", data.field , function(result) {
                    if (result.flag == 0 && result.code == 200) {
                        layer.alert('修改成功.', {
